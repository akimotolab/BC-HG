...
def clip_grads_per_sample_norm(grads: Any, max_norm: Optional[float]):
    ...
    def clip_leaf(leaf):
        ...
        is_valid_max_norm = (max_norm is not None) and (max_norm > 0)
        clipped = jax.lax.cond(
            is_valid_max_norm,
            apply_clipping,
            no_clipping
        )
...
def calculate_advantage_gradient(
    ...
    for i, param_name in enumerate(param_names):
        ...
        def create_transition_value_grad_func(
                ll_value_fn: Callable, il_params: jnp.ndarray, aa_params: jnp.ndarray, grad_idx: int
            ) -> Callable:
            log_transition_prob_grad = jax.grad(log_transition_func, argnums=[grad_idx])
            
            def transition_value_grad_func(
                    s: jnp.ndarray, a: jnp.ndarray, n_s: jnp.ndarray, p: jnp.ndarray
                ) -> jnp.ndarray:
...
def create_update_step(
    ...
    def update_step(carry, step_input):
        ...
        br_grad_IL = (
            ...
            * jnp.expand_dims(UL_reward_discounted, -1)
        ...
        metrics = {
            ...
            "Q_UL": jnp.nanmean(UL_reward_discounted),
