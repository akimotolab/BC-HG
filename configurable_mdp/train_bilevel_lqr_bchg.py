...
def calculate_advantage_gradient(
    ...
    for i, param_name in enumerate(param_names):
        ...
        grads[param_name] = LL_q_grad

    LL_q_grad_concat = jnp.concatenate(
        [grads[param_name] for param_name in param_names],
        axis=2
    )  # Shape: (n_steps, num_envs, dim_IL_param + dim_AA_param)

    return grads, LL_q_grad_concat
...
def create_update_step(
    ...
    def update_step(carry, step_input):
        ...
        UL_reward, UL_reward_discounted = calculate_discounted_rewards(
            env_params_train_carry,  # EnvParams for env.upper_level_reward
        ...
        # Upper-level q-value estimation
        ...
        LL_q_grad, LL_q_grad_concat = calculate_advantage_gradient(
            env,
            env_params_train_carry,
            traj_batch,
            LL_value_fn,
        ...
        # Upper-level Benefit estimation
        Benefit = UL_q_value_estimate - UL_value_estimate  # Shape: (n_steps, num_envs)
        br_grad_IL = (
            LL_q_grad["insulation_level"]
            * jnp.expand_dims(Benefit, -1)
            / config["lower_optimisation"]["reg_lambda"]
        )  # Shape: (n_steps, num_envs, dim_IL_param)
        br_grad_AA = (
            LL_q_grad["airflow_adjustment"]
            * jnp.expand_dims(Benefit, -1)
            / config["lower_optimisation"]["reg_lambda"]
        )  # Shape: (n_steps, num_envs, dim_AA_param)
        ...
        metrics = {
            ...
            "LL_Q_grad_mean": jnp.nanmean(LL_q_grad_concat, axis=(0,1)),
