"""Proposal
    - Upper-level Q table is updated online by SARSA on sampled transitions.
    - Upper-level value is computed as policy expectation of the current Q table.
    - Lower-level advantage-gradient is estimated from Monte Carlo discounted
      returns and combined with the UL reward-gradient term.
"""
...
if __name__ == "__main__":
    ...
    def run_experiment(
        ...
        UL_Q_value = jax.random.normal(rng, (n_states, n_actions))  # (n_states, n_actions), randomly initialized
        return jax.lax.scan(
            update_step,
