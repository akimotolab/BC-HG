"""HPGD (TD-based)
    ...
    - Value-network parameters are not carried over inside each outer iteration;
      each iteration starts from the same carried estimator state.
"""
import jax
...
def calculate_discounted_rewards_with_next_state(
    reward_function_params,
    reward_function: Callable,
    traj_batch: Transition,
    discount_factor: float,
    ...
    """
    Calculate the discounted rewards for a trajectory batch when the reward function depends on the next state
    :param reward_function_params:
    :param reward_function:
    :param traj_batch:
    :param discount_factor:
    :param initial_value: Initial value for the discounted rewards, matching the shape of the output of reward_function
    :param next_state: Next state for the trajectory batch
    :return: Discounted rewards, matching the shape of the initial_value
        The returned array has shape (n_steps, num_envs, pyTree structure of initial_value)
    """
    ...
    def _get_discounted_reward(
        ...
        # when reward_function depends on next_state
        transition, next_state = scan_input
        reward = jax.vmap(
            reward_function,
            in_axes=(0, 0, 0, None),
        )(
            transition.state, transition.action, next_state, reward_function_params,
        )  # Shape: (num_envs, Optional[params_dim] )
        done = transition.done.astype(jnp.float32)  # Shape: (num_envs,)
        rolling_discounted_rewards = jax.tree_map(
            lambda x, y: (
...
def estimate_value_function(
    ...
    rewards_max = jnp.where(
        normalize_rewards,
        jnp.max(jnp.abs(jnp.where(
            jnp.isnan(rewards_reshaped), 0.0, rewards_reshaped
        )), axis=0, keepdims=True),
        jnp.ones((1, *rewards_reshaped.shape[1:]))
...
def calculate_advantage_gradient(
    ...
    Note: currently commented out parts related to reward gradient calculation,
    as the upper-level reward does not depend on the upper-level parameters.
    """
    ...
    # ):
    #     """Auxiliary function to calculate the reward as a function of the upper-level parameters"""
    #     reward_params_tmp = params.reward_params.replace() # No upper-level parameters in reward
    #     return env.reward(state, action, reward_params_tmp)

    def log_transition_func(
        state: EnvState, 
        action: jax.Array, 
        next_state: EnvState, 
        params: EnvParams,
        ...
        """Auxiliary function to calculate the transition log probability as a function of the upper-level parameters"""
        has_nan = jnp.any(jnp.isnan(next_state.temperature_deviations))

        def compute_logprob():
            transition_params_tmp = params.transition_params.replace(
            ...
            mean = env.transition(
                state, 
                action, 
                transition_params_tmp
            ).temperature_deviations
            std = transition_params_tmp.transition_std
            lower_bound, upper_bound = transition_params_tmp.temperature_range
            logprobs = jax.scipy.stats.truncnorm.logpdf(
                next_state.temperature_deviations,
                a=lower_bound,
                b=upper_bound,
                loc=mean,
                scale=std,
        ...
        return jnp.where(has_nan, jnp.nan, compute_logprob())

    state = traj_batch.state
    temp_next = jnp.where(
        jnp.expand_dims(traj_batch.done, -1),
        jnp.full_like(state.temperature_deviations, jnp.nan, dtype=state.temperature_deviations.dtype),
        jnp.roll(state.temperature_deviations, shift=-1, axis=0),
    )  # Shape: (n_steps, num_envs, dim_obs)
    time_next = jnp.where(
        traj_batch.done,
        jnp.full_like(state.time, -1, dtype=state.time.dtype),
        jnp.roll(state.time, shift=-1, axis=0),
    )  # Shape: (n_steps, num_envs)
    next_state = EnvState(
        temperature_deviations=temp_next,
        time=time_next
    )  # Shape: (n_steps, num_envs, ...)

    # Loop over each parameter
    grads = {}
    param_names = ["insulation_level", "airflow_adjustment"]
    param_indices = [3, 4]    # argnums for jax.grad(reward_func) of ["insulation_level", "airflow_adjustment"]
    logprob_indices = [4, 5]  # argnums for jax.grad(log_transition_func) of ["insulation_level", "airflow_adjustment"]
    for i, param_name in enumerate(param_names):
        reward_idx = param_indices[i]
        logprob_idx = logprob_indices[i]
        # CALCULATE THE GRADIENT OF THE DISCOUNTED REWARDS W.R.T. THE UPPER-LEVEL PARAMETERS
        # reward_grad = jax.grad(reward_func, argnums=[reward_idx])
        ...
        #     traj_batch,
        #     config_lower_optimisation["discount_factor"],
        #     initial_value=jnp.zeros((config_upper_optimisation["num_envs"], param.shape[0])),
        # )  # Shape: (n_steps, num_envs, dim_param)

        # CALCULATE THE GRADIENT OF THE TRANSITION LOG-PROBABILITY W.R.T. THE UPPER-LEVEL PARAMETERS
        def create_transition_value_grad_func(ll_value_fn, il_params, aa_params, grad_idx):
            log_transition_prob_grad = jax.grad(log_transition_func, argnums=[grad_idx])
            def transition_value_grad_func(s, a, n_s, p):
                grad_result = log_transition_prob_grad(s, a, n_s, p, il_params, aa_params)
                grad_weights = grad_result[0]["params"]["weights"]
                ll_value = ll_value_fn(n_s.temperature_deviations)
                transition_value_grad = grad_weights * ll_value
                transition_value_grad = jnp.nan_to_num(transition_value_grad, nan=0.0)  # Filling NaNs with zeros, NaNs occur at time steps with no next state
                return transition_value_grad
            return transition_value_grad_func

        transition_value_grad_func = create_transition_value_grad_func(
            lower_level_value_fn, 
            upper_level_train_states["insulation_level"].params, 
            upper_level_train_states["airflow_adjustment"].params, 
            logprob_idx
        )
        LL_transition_grad, LL_discounted_transition_grad = calculate_discounted_rewards_with_next_state(
            env_params,
            transition_value_grad_func,
            traj_batch,
            config_lower_optimisation["discount_factor"],
            initial_value=jnp.zeros(
                (config_upper_optimisation["num_envs"], 
                 env_params.transition_params.insulation_level.shape[0] if param_name == "insulation_level" 
                 else env_params.transition_params.airflow_adjustment.shape[0])
            ),
        ...
        # LL_q_grad = (
        #     LL_discounted_rewards_grad
        #     + config_lower_optimisation["discount_factor"]
        #       * LL_discounted_transition_grad
        # )  # Shape: (n_steps, num_envs, dim_param)
        # LL_q_grad_before_discounted_sum = (
        #     LL_rewards_grad 
        #     + config_lower_optimisation["discount_factor"]
        #       * LL_transition_grad
        # )  # Shape: (n_steps, num_envs, dim_param)
        ...
        grads[param_name] = {
            "LL_q_grad": LL_q_grad,
            "LL_q_grad_before_discounted_sum": LL_q_grad_before_discounted_sum
        }

    # Estimate gradients of the value function
    LL_q_grad_before_discounted_sum = jnp.concatenate(
        [grads[param_name]["LL_q_grad_before_discounted_sum"] for param_name in param_names],
        axis=2
    )  # Shape: (n_steps, num_envs, dim_IL_param + dim_AA_param)
    value_model_params = config_upper_optimisation["value_model_params"]
    _, LL_value_grad, _ = estimate_value_function(
        X,
        X_next,
        LL_q_grad_before_discounted_sum,
        value_function_estimator,
        num_steps=value_model_params["num_training_steps"],
        discount_factor=config_lower_optimisation["discount_factor"],
    ...
    LL_advantage_grad = {}
    for i, param_name in enumerate(param_names):
        LL_q_grad = grads[param_name]["LL_q_grad"]
        dim_param = LL_q_grad.shape[-1]
        LL_advantage_grad[param_name] = (
            LL_q_grad - LL_value_grad[ :, :, i*dim_param:(i+1)*dim_param ]
        )  # Shape: (n_steps, num_envs, dim_param)

    LL_q_grad = jnp.concatenate(
        [grads[param_name]["LL_q_grad"] for param_name in param_names],
        axis=2
    )  # Shape: (n_steps, num_envs, dim_IL_param + dim_AA_param)

    return LL_advantage_grad, LL_q_grad, LL_value_grad
...
def create_update_step(
    ...
    def update_step(carry, step_input):
        ...
        policy = RegularizedLQRPolicy(
            Ks=lower_level_states_output.Ks,
            Sigma=lower_level_states_output.Sigma,
            sqrtSigma=lower_level_states_output.sqrtSigma,
        )
        ...
        # Data preparation for q-value estimation
        X = jnp.concatenate(
            [traj_batch.obs, traj_batch.action], axis=-1
        )  # Shape: (n_steps, num_envs, obs_dim+act_dim)
        X_next = jnp.where(
            jnp.expand_dims(traj_batch.done, axis=list(range(2, X.ndim))),
            jnp.full_like(X, jnp.nan, dtype=X.dtype),
            jnp.roll(X, shift=-1, axis=0),
        )
        # Estimate upper-level q-value function
        value_model_params = config_upper_optimisation["value_model_params"]
        UL_q_value_fn, UL_q_value_estimate, _ = estimate_value_function(
            X,
            X_next,
            UL_reward,
            value_function_estimators_carry["UL_q_value"],
            num_steps=value_model_params["num_training_steps"],
            discount_factor=config_upper_optimisation["discount_factor"],
            normalize_rewards=False,
            l2_reg=0.0
        )  # UL_q_value_estimate.shape: (n_steps, num_envs)
        # Estimate upper-level value function by sampling actions from the current policy
        obs = traj_batch.obs.reshape(-1, traj_batch.obs.shape[-1])  # Shape: (n_steps*num_envs, obs_dim)
        
        def sample_q_value(rng_carry, unused):
            rng_carry, _rng = jax.random.split(rng_carry)
            rng_keys = jax.random.split(_rng, obs.shape[0])
            actions = policy.get_actions(obs, rng_keys)  # Shape: (n_steps*num_envs, act_dim)
            X_input = jnp.concatenate([obs, actions], axis=-1)
            q_values = UL_q_value_fn.apply_fn(UL_q_value_fn.params, X_input)  # Shape: (n_steps*num_envs, 1)
            return rng_carry, q_values.reshape(-1)  # Shape: (n_steps*num_envs,)

        rng_carry, _rng = jax.random.split(rng_carry)
        _, q_values_samples = jax.lax.scan(
            sample_q_value,
            _rng,
            None,
            length=config_upper_optimisation["num_sample_value_estimation"],
        )  # Shape: (num_sample_value_estimation, n_steps*num_envs)
        UL_value_estimate = jnp.mean(q_values_samples, axis=0)  # Shape: (n_steps*num_envs,)
        UL_value_estimate = UL_value_estimate.reshape(
            traj_batch.obs.shape[0], traj_batch.obs.shape[1]
        )  # Shape: (n_steps, num_envs)
        ...
        # LL_reward_discounted: lower-level value
        LL_advantage_grad, LL_q_grad, LL_value_grad = calculate_advantage_gradient(
            env,
            env_params_train_carry,
            traj_batch,
            X,
            ...
            LL_value_fn,
            upper_level_train_states_carry,
            value_function_estimators_carry["LL_value_grad"],
            config,
        )  # {"insulation_level": (n_steps, num_envs, dim_IL_param), "airflow_adjustment": (n_steps, num_envs, dim_AA_param)}
        br_grad_IL = (
            LL_advantage_grad["insulation_level"]
            * jnp.expand_dims(UL_q_value_estimate, -1)
            / config["lower_optimisation"]["reg_lambda"]
        )  # Shape: (n_steps, num_envs, dim_IL_param)
        br_grad_AA = (
            LL_advantage_grad["airflow_adjustment"]
            * jnp.expand_dims(UL_q_value_estimate, -1)
            / config["lower_optimisation"]["reg_lambda"]
        )  # Shape: (n_steps, num_envs, dim_AA_param)
        ...
        upper_level_grad_IL = (
            jnp.nansum(
                upper_level_grad_IL * jnp.expand_dims(traj_batch_discounting, -1), axis=(0, 1)
            )
            / jnp.clip(num_episodes, a_min=1)
        )  # Shape: (dim_IL_param,)
        br_grad_IL = (
            jnp.nansum(
                br_grad_IL * jnp.expand_dims(traj_batch_discounting, -1), axis=(0, 1)
            )
            / jnp.clip(num_episodes, a_min=1)
        )  # Shape: (dim_IL_param,)
        IL_grad = upper_level_grad_IL + br_grad_IL  # Shape: (dim_IL_param,)
        # Airflow Adjustment Grad
        upper_level_grad_AA = (
            jnp.nansum(
                upper_level_grad_AA * jnp.expand_dims(traj_batch_discounting, -1), axis=(0, 1)
            )
            / jnp.clip(num_episodes, a_min=1)
        )  # Shape: (dim_AA_param,)
        br_grad_AA = (
            jnp.nansum(
                br_grad_AA * jnp.expand_dims(traj_batch_discounting, -1), axis=(0, 1)
            )
            / jnp.clip(num_episodes, a_min=1)
        )  # Shape: (dim_AA_param,)
        AA_grad = upper_level_grad_AA + br_grad_AA  # Shape: (dim_AA_param,)
        ...
        metrics = {
            ...
            "Q_UL": jnp.nanmean(UL_q_value_estimate),
            ...
            "insulation_level_UL_grad": upper_level_grad_IL,
            "insulation_level_BR_grad": br_grad_IL,
            "airflow_adjustment_grad": AA_grad,
            "airflow_adjustment_UL_grad": upper_level_grad_AA,
            "airflow_adjustment_BR_grad": br_grad_AA,
            "LL_Q_grad_mean": jnp.nanmean(LL_q_grad, axis=(0,1)),
            "LL_value_grad_mean": jnp.nanmean(LL_value_grad, axis=(0,1)),
...
if __name__ == "__main__":
    ...
    def run_experiment(
        ...
        act_dim = env_params_exp.transition_params.num_hvac_units
        ...
        if checkpoint is None:
            ...
            value_function_estimators = {
                "LL_value_grad": create_train_state_value_model(
                    key=_rng1,
                    input_dim=obs_dim,
                    output_dim=(n_dim_insulation_level+n_dim_airflow_adjustment),
                    layer_size=config_value_model["layer_size"],
                    optimizer_params=config_value_model["optimizer_params"],
                ),
                "UL_q_value": create_train_state_value_model(
                    key=_rng2,
            ...
            value_function_estimators = {
                "LL_value_grad": restore_train_state_value_model(
                    value_function_estimators_dict["LL_value_grad"],
                    output_dim=(n_dim_insulation_level+n_dim_airflow_adjustment),
                    layer_size=config_value_model["layer_size"],
                    optimizer_params=config_value_model["optimizer_params"],
                ),
                "UL_q_value": restore_train_state_value_model(
                    value_function_estimators_dict["UL_q_value"],
                    output_dim=1,
                    layer_size=config_value_model["layer_size"],
                    optimizer_params=config_value_model["optimizer_params"],
                ),
