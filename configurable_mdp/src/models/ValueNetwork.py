...
def restore_train_state(
    ...
    """
    Restore the training state for the value network from a saved state
    :param restored_state: Dictionary containing the saved state
    :param output_dim: Output dimension of the network
    :param layer_size: Sizes of the hidden layers
    :param activation: Activation function to use
    :param optimizer_params: Parameters for the optimizer
    :return: Restored TrainState
    """
    model = ValueNetwork(
        layer_sizes=layer_size,
        activation=activation,
    ...
    adam_dict = restored_state["opt_state"][1][0]
    adam_state = optax.ScaleByAdamState(
        count=adam_dict["count"],
        mu=adam_dict["mu"],
        nu=adam_dict["nu"],
    )
    opt_state = (
        optax.EmptyState(),
        (adam_state, optax.EmptyState())
    )
    return TrainState(
