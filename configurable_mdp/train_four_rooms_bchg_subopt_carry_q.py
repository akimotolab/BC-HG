...
def create_update_step(
    ...
    def update_step(carry, reg_lambda: float) -> Tuple:
        (rng, env_params_train_carry, incentive_train_state_carry, UL_Q_value, q_carry) = carry
        ...
        br_response_grad = (
            LL_Q_grad * jnp.expand_dims(UL_advantage, -1) / reg_lambda
        )  # (n_steps, n_envs, |params|)
        br_response_grad_baseline = (
            LL_value_grad * jnp.expand_dims(UL_advantage, -1) / reg_lambda
        )  # (n_steps, n_envs, |params|)
        ...
        br_response_grad_baseline = (
            jnp.sum(
                br_response_grad_baseline * traj_batch_discount_factor, axis=(0, 1),
            )
            / jnp.clip(n_episodes_advantage, a_min=1)  # Avoid division by zero
        )
        ...
        # Compute cosine similarity between (br_response_grad - br_response_grad_baseline) and br_response_grad
        br_response_grad_A_Q_cosine_similarity = compute_cosine_similarity(
            (br_response_grad - br_response_grad_baseline), br_response_grad
        )
        ...
        info = {
            ...
            "policy_grad_baseline_norm": jnp.linalg.norm(br_response_grad_baseline),
            "policy_grad_A_Q_cosine_similarity": br_response_grad_A_Q_cosine_similarity,
        ...
        return (rng, env_params_train_carry, incentive_train_state_carry, UL_Q_value, q_carry), info
...
if __name__ == "__main__":
    ...
    def run_experiment(
        ...
        return jax.lax.scan(
            update_step,
            (rng, env_params_exp, incentive_train_state_exp, UL_Q_value, q_carry),
            reg_lambda,
            n_iter,
        )
    ...
    _, env_params, incentive_train_state, _, _ = carry_out
