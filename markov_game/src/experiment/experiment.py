...
import dateutil.tz
import dowel
from dowel import logger, Logger, TabularInput
import numpy as np
...
class ExperimentContext:
    ...
    def __init__(self, *, snapshot_dir, snapshot_mode, snapshot_gap, x_axis):
...
class ExperimentTemplate:
    ...
    def _make_context(cls, options, **kwargs):
        ...
        tabular_log_file = os.path.join(log_dir, 'follower/progress.csv')
        tensorboard_log_dir = os.path.join(log_dir, 'follower')
        ...
        logger.add_output(
            dowel.TensorBoardOutput(tensorboard_log_dir, x_axis=options['x_axis']))
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.function is None:
            ...
            loaded_data = kwargs.pop('loaded_data') if 'loaded_data' in kwargs else None
            ctxt = self._make_context(self._get_options(*args), **kwargs)
            if loaded_data is not None:
                kwargs['loaded_data'] = loaded_data
            ...
            if isinstance(o, dict):
                ...
                if obj_dict is not None:
                    data = {k: self.default(v) for (k, v) in obj_dict.items() 
                            if not isinstance(v, (Logger, TabularInput))}
                else:
                    data = {
