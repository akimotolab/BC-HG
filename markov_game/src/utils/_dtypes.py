...
class EpisodeBatch(TimeStepBatch):
    ...
    Attributes:
        ...
        last_leader_actions (numpy.ndarray): A numpy array of shape
            :math:`(N, A^*)` containing the last leader action of each episode.
    ...
    last_leader_actions: np.ndarray
    lengths: np.ndarray

    def __init__(self, env_spec, episode_infos, observations,
                 last_observations, last_leader_actions, 
                 actions, rewards, env_infos, agent_infos,
        ...
        if not _space_soft_contains(env_spec.observation_space,
            ...
            raise ValueError(f'last_observations must have the same '
                             ...
                             f'{last_observations[0].shape} entries')
        if not isinstance(last_leader_actions, np.ndarray):
            raise ValueError(
                f'last_leader_actions is not of type {np.ndarray!r}')
        if last_leader_actions.shape[0] != n_episodes:
            raise ValueError(
                f'last_leader_actions has batch size '
                f'{last_leader_actions.shape[0]} but must have '
                f'batch size {n_episodes} to match the number of episodes')
        if not _space_soft_contains(env_spec.leader_action_space,
                                    last_leader_actions[0]):
            raise ValueError(f'last_leader_actions must have the same '
                             f'number of entries as there are episodes '
                             f'({n_episodes}) but got data with shape '
                             f'{last_leader_actions[0].shape} entries')

        object.__setattr__(self, 'last_observations', last_observations)
        object.__setattr__(self, 'last_leader_actions', last_leader_actions)
        ...
        check_timestep_batch(
            self,
            np.ndarray,
            ignored_fields={'next_observations', 'next_leader_actions', 'episode_infos'})
    ...
    def concatenate(cls, *batches):
        ...
        return cls(
            ...
            last_leader_actions=np.concatenate(
                [batch.last_leader_actions for batch in batches]),
            actions=np.concatenate([batch.actions for batch in batches]),
            rewards=np.concatenate([batch.rewards for batch in batches]),
            env_infos=env_infos,
            agent_infos=agent_infos,
    ...
    def split(self):
        ...
        for i, (start, stop) in enumerate(self._episode_ranges()):
            eps = EpisodeBatch(
                ...
                last_leader_actions=np.asarray([self.last_leader_actions[i]]),
    ...
    def to_list(self):
        ...
        Returns:
            list[dict[str, np.ndarray or dict[str, np.ndarray]]]: Keys:
                ...
                * next_observations (np.ndarray): Non-flattened array of
                    ...
                    observed by the agent after taking actions[i].
                * next_leader_actions (np.ndarray): Non-flattened array of
                    next leader actions. Has shape (T, A^*). next_leader_actions[i]
                    was observed by the agent after taking actions[i].
        ...
        for i, (start, stop) in enumerate(self._episode_ranges()):
            episodes.append({
                ...
                'next_leader_actions':
                np.concatenate((self.env_infos['leader_action'][1 + start:stop],
                                [self.last_leader_actions[i]])),
                'actions':
    ...
    def from_list(cls, env_spec, paths):
        ...
        if all(len(path['env_infos']['leader_action']) == length + 1
               for (path, length) in zip(paths, lengths)):
            last_leader_actions = np.asarray(
                [p['env_infos']['leader_action'][-1] for p in paths])
            env_infos = {
                k: np.concatenate([p['env_infos'][k][:-1] for p in paths])
                for k in paths[0]['env_infos']
            }
        else:
            # The number of observations and timesteps must match.
            env_infos = {
                k: np.concatenate([p['env_infos'][k] for p in paths])
                for k in paths[0]['env_infos']
            }
            if paths[0].get('next_leader_actions') is not None:
                last_leader_actions = np.asarray(
                    [p['next_leader_actions'][-1] for p in paths])
            else:
                last_leader_actions = np.asarray(
                    [p['env_infos']['leader_action'][-1] for p in paths])
        ...
        return cls(env_spec=env_spec,
                   ...
                   last_leader_actions=last_leader_actions,
    ...
    @property
    def next_leader_actions(self):
        r"""Get the leader actions seen after actions are performed.

        In an :class:`~EpisodeBatch`, next_leader_actions don't need to be stored
        explicitly, since the next leader action is already stored in the batch.

        Returns:
            np.ndarray: The "next_leader_actions" with shape
                :math:`(N \bullet [T], A^*)`

        """
        return np.concatenate(
            tuple([
                np.concatenate((eps.env_infos['leader_action'][1:], eps.last_leader_actions))
                for eps in self.split()
            ]))
    ...
    @property
    def padded_next_leader_actions(self):
        """Padded next_leader_actions array.

        Returns:
            np.ndarray: Array of shape :math:`(N, max_episode_length, A^*)`

        """
        return pad_batch_array(self.next_leader_actions, self.lengths,
                               self.env_spec.max_episode_length)
...
def check_timestep_batch(batch, array_type, ignored_fields=()):
    ...
    fields = {
        ...
        for field in [
            ...
            'next_observations', 'next_leader_actions', 'step_types', 'agent_infos', 'episode_infos',
            'env_infos'
    ...
    for field, value in fields.items():
        if field in [
                ...
                'next_leader_actions', 'step_types'
        ]:
        ...
        if hasattr(value, 'shape'):
            ...
            if env_spec and field == 'next_leader_actions':
                if not _space_soft_contains(env_spec.leader_action_space, value[0]):
                    raise ValueError(
                        f'Each {field[:-1]} has shape {value[0].shape} '
                        f'but must match the leader_action_space '
                        f'{env_spec.leader_action_space}')
