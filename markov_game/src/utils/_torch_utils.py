...
def compute_divergence_custom(p, q):
    p_ = np.zeros(p.shape)
    for idx_s, idx_a in enumerate(p.argmax(axis=1)):
        p_[idx_s, idx_a] = 1
    div = np.mean((p_ - q)**2)
    return div


def compute_kl_divergence(p, q, p_is_q_table=False, e=0.05):
    if e == 0:
        e = 1e-7
    if p_is_q_table:
        div = 0
        s_space_size = p.shape[0]
        a_space_size = p.shape[1]
        for idx_s in range(s_space_size):
            p_dist = np.ones(a_space_size) * e / a_space_size
            p_dist[p[idx_s].argmax()] += 1-e
            div += np.sum(p_dist * (np.log(p_dist) - np.log(q[idx_s])))
        div /= s_space_size
    else:
        log_pq = np.log(p) - np.log(q)
        div = np.mean(np.sum( p * log_pq, axis=1 ))
    return div
...
def correlation_coefficient(tensor_a, tensor_b):
    ...
    Args:
        tensor_a (torch.Tensor): A tensor of shape (d, 1).
        tensor_b (torch.Tensor): A tensor of shape (d, 1).
    ...
    return correlation.detach().item()


def pad_and_stack(subseq_list, dtype=torch.float32, device=None, with_mask=False):
    """Pad and stack a list of sequences.
    Args:
        subseq_list (list): List of sequences to pad and stack.
        dtype (torch.dtype): Data type of the output tensor.
        device (torch.device or None): Device to create the output tensor on.
            If None, the default device will be used.
    Returns:
        torch.Tensor: Padded and stacked tensor.

    Note: the code below is teh former non-optimized version:
        subsequences = {key: [torch.as_tensor(subseq).float().to(global_device()) for subseq in subseq_list] 
                        for key, subseq_list in sampled_subsequences.items()}
    e.g.)
    subsequences = {
            key: pad_and_stack(subseq_list, device=global_device(), with_mask=(key == 'observation'))
            for key, subseq_list in {'observation': sampled_subsequences['observation'], 
    ...
    if with_mask:
        ...
        for i, subseq in enumerate(subseq_list):
            subseq_arr = np.asarray(subseq)
            padded[i, :len(subseq_arr)] = subseq_arr
            is_padding[i, :len(subseq_arr)] = True
        padded_tensor = torch.from_numpy(padded).to(dtype=dtype, device=device)
        is_padding_tensor = torch.from_numpy(is_padding).to(device=device)
    else:
        for i, subseq in enumerate(subseq_list):
            subseq_arr = np.asarray(subseq)
            padded[i, :len(subseq_arr)] = subseq_arr
        padded_tensor = torch.from_numpy(padded).to(dtype=dtype, device=device)
        is_padding_tensor = None
    return padded_tensor, is_padding_tensor
...
def reset_optimizer(optimizer, module, lr):
    optimizer_class = type(optimizer)
    return optimizer_class(module.parameters(), lr=lr)
    

def torch_cg(A, b, x0=None, tol=1e-5, maxiter=None, M=None, atol=0.0):
    """
    PyTorch version of Conjugate Gradient method (SciPy-like specification).
    A: [n, n] torch.Tensor or matvec function
    b: [n] torch.Tensor
    x0: Initial value [n] torch.Tensor (optional)
    tol: Relative tolerance
    maxiter: Maximum number of iterations (default: n*10)
    M: Preconditioner matrix or matvec function (default: identity operator)
    atol: Absolute tolerance
    Returns: x [n] torch.Tensor, info (0: converged, >0: not converged within maxiter, <0: abnormal termination)
    """
    if callable(A):
        matvec = A
    else:
    ...
    if M is None:
        ...
        psolve = lambda v: torch.linalg.solve(M, v)
    ...
    for i in range(maxiter):
        Ap = matvec(p)
        alpha = rzold / torch.dot(p, Ap)
        x = x + alpha * p
        r = r - alpha * Ap

        # Relative residual check
        res_norm = torch.norm(r)
        if res_norm <= tol * normb + atol:
            # Double-check by recomputing
            r_check = b - matvec(x)
            if torch.norm(r_check) <= tol * normb + atol:
                info = 0
                break

        z = psolve(r)
        rznew = torch.dot(r, z)
        if torch.isnan(rznew) or torch.isinf(rznew):
            info = -1
            break
