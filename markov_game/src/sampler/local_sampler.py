...
class LocalSampler(Sampler):
    ...
    def __init__(
        ...
        self._agents = self._factory.prepare_worker_messages(agents)  # Joint policy
        self._envs = self._factory.prepare_worker_messages(
            envs, preprocess=copy.deepcopy) # deepcopy? or cloudpickle.loads(cloudpickle.dumps(self._env))?
        self._workers = [
    ...
    def obtain_samples(self, 
        ...
        Args:
            ...
            env_update (object): Value which will be passed into the
                ...
                in, it must have length exactly `factory.n_workers`, and will
                be spread across the workers.
            init_state (np.ndarray): Initial state to reset the environment
                to. If None, environment is reset randomly.
            init_follower_action (np.ndarray): Initial action of the follower
                to set the environment to. If None, environment is reset
                randomly.
            get_actions_kwargs (dict or None): Optional keyword arguments
                passed to `JointPolicy.get_action` / `JointPolicy.get_actions`.
    ...
    def obtain_exact_episodes(self,
        ...
        for worker in self._workers:
            for _ in range(n_eps_per_worker):
                batch = worker.rollout(init_state, 
                                       init_follower_action,
                                       get_actions_kwargs)
                batches.append(batch)
        samples = EpisodeBatch.concatenate(*batches)
