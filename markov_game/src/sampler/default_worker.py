...
class DefaultWorker(Worker):
    ...
    Attributes:
        agent (JointPolicy or None): The worker's agent.
    ...
    def __init__(
        ...
        self._lengths = []
        self._prev_obs = None
        self._prev_acts = None
        self._prev_agent_info = None
        self._eps_length = 0
    ...
    def start_episode(self, 
        ...
        """Begin a new episode."""
        self._eps_length = 0
        self._prev_obs, episode_info = self.env.reset(init_state)
        self._prev_acts, self._prev_agent_info = self.agent.get_action(
            self._prev_obs,
            deterministic_leader=deterministic_l,
            deterministic_follower=deterministic_f,
            explorate_leader=deterministic_l,
            explorate_follower=deterministic_f,
            zero_leader=zero_action_l,
            zero_follower=zero_action_f
        )
        if init_follower_action is not None:
            self._prev_acts[1] = init_follower_action
        for k, v in episode_info.items():
            self._episode_infos[k].append(v)
        self._episode_infos['init_state'].append(init_state)
        self._episode_infos['init_follower_action'].append(init_follower_action)
    ...
    def step_episode(self, 
        ...
        if self._eps_length < self._max_episode_length:
            a = self._prev_acts
            for k, v in self._prev_agent_info.items():
                self._agent_infos[k].append(v)
            es = self.env.step(a)
            next_obs = es.observation
            next_acts, agent_info = self.agent.get_action(
                next_obs,
                deterministic_leader=deterministic_l,
                deterministic_follower=deterministic_f,
            ...
            es.env_info['time_step'] = self._eps_length
            es.env_info['next_leader_action'] = next_acts[0]
            es.env_info['next_agent_info'] = agent_info
            self._env_steps.append(es)
            self._eps_length += 1
            self._prev_obs = next_obs
            self._prev_acts = next_acts
            self._prev_agent_info = agent_info

            # If next_state is not the terminal state, return False
            if not es.terminal: return False
        # If the episode reached max length or next_state is terminal, return True
    ...
    def collect_episode(self):
        ...
        return EpisodeBatch(env_spec=self.env.spec,
                            ...
                            actions=np.asarray(actions),  # follower's actions
                            rewards=np.asarray(rewards),  # follower's rewards
                            step_types=np.asarray(step_types, dtype=StepType),
                            env_infos=dict(env_infos),    # dict('leader_action', 'target_reward', 'next_leader_action', 'next_agent_info')
                            agent_infos=dict(agent_infos),
                            lengths=np.asarray(lengths, dtype='i'))

    def rollout(self, init_state=None, init_follower_action=None, get_actions_kwargs=None):
        ...
        """
        if get_actions_kwargs is None:
            get_actions_kwargs = {}
        self.start_episode(init_state, init_follower_action, **get_actions_kwargs)
        while not self.step_episode(**get_actions_kwargs):
            pass
