...
import gym
import numpy as np
from garage import EnvStep, StepType

from ._environment import Environment, GlobalEnvSpec
...
class GymEnv(Environment):
    ...
    def __init__(self, 
        ...
        self._leader_action_space = akro.from_gym(self._env.leader_action_space)
        self._observation_space = akro.from_gym(self._env.observation_space,
                                                is_image=is_image)
        self._spec = GlobalEnvSpec(action_space=self._action_space,
        ...
        # If True, always return zero leader actions
        self._zero_leader = zero_leader
    ...
    @property
    def frames(self):
        """list: A list of rendered frames."""
        return self._frames

    @property
    def zero_leader(self):
        """bool: Whether to always return zero leader actions."""
        return self._zero_leader
    ...
    def reset(self, initial_state=None):
        ...
        """
        first_obs = self._env.reset(initial_state=initial_state)
        self._step_cnt = 0
        self._env_info = None
        self._frames = []
    ...
    def set_state(self, state):
        assert hasattr(self.env, 'state'), 'The wrapped env of GymEnv does not have state'
    ...
    def seed(self, seed=None):
        ...
        self._leader_action_space.seed(seed)
    ...
    def step(self, action):
        ...
        Args:
            action (list[np.ndarray] or tuple[np.ndarray, np.ndarray]): 
            A list or tuple of leader's and follower's actions provided by the agents.
            action[0] (np.ndarray): The leader's action.
            action[1] (np.ndarray): The follower's action.
        ...
        if not isinstance(action, (list, tuple)) or len(action) != 2:
            raise TypeError("action must be a list or tuple with two elements")
        
        if self._zero_leader:
            action[0] = np.zeros_like(action[0])

        observation, reward, done, info = self._env.step(action)  # action[0]=leader_action, action[1]=follower_action

        if self._zero_leader:
            info['leader_action'] = action[0]
            info['zero_leader'] = True

        if self._visualize:
            self._frames.append(self._env.render(mode=self._render_mode))
        ...
        return EnvStep(env_spec=self.spec,
                       action=action[1],  # follower's action
                       reward=reward,
                       observation=observation,
                       env_info=info,     # info['leader_action'] = action[0],  info['target_reward'] = target_reward (, info['zero_leader'] = True)
                       step_type=step_type)
    ...
    def __setstate__(self, state):
        ...
        if '_zero_leader' in state:
            self._zero_leader = state['_zero_leader']
